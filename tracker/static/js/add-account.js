// Function to fetch updated account data without page reload
function fetchUpdatedAccountData() {
    ...
    const monthName = monthTitle.textContent.replace(' Details', '');
    ...
    .then(response => {
        if (response.ok) {
            // Instead of trying to update the complex DOM, just reload the page
            // but do it after a success message is shown
            setTimeout(function() {
                location.reload();
            }, 1500);
        } else {
    ...
    .catch(error => {
        ...
        // If we can't fetch the data, just reload the page as a fallback
        setTimeout(function() {
            location.reload();
        }, 1500);
    });
...
function createAddAccountModal() {
    ...
    document.getElementById('addAccountForm').addEventListener('submit', function(e) {
        ...
        .then(response => {
            if (response.ok) {
                // Hide the modal but keep the month details open
                document.getElementById('addAccountModal').classList.add('hidden');
                
                // Show success notification
                showNotification('Account added successfully', 'success');
                
                // Fetch the updated account data without page reload
